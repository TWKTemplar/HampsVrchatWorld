Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/80601e3ece400074c97a2e04921e4a6d"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(14.75442,14.75442,14.75442,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(14.75442,14.75442,14.75442,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(14.75442,14.75442,14.75442,1).rgb, float(0));
				#endif
				if (float(0))
				{
				...
				emissionColor0 = hueShift(emissionColor0, frac(float(0.739) + float(0) * _Time.x) * float(0));
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
