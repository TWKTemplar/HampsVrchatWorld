...
float3 get4VertexLightsColFalloff(inout VertexLightInformation vLight, float3 worldPos, float3 normal, inout float4 vertexLightAtten)
{
    float3 lightColor = 0;
        ...
        //atten = clamp(atten,float4(_MinLightLevel,_MinLightLevel,_MinLightLevel,1),float3(1,1,1,1));
        atten=atten*(1-_MinLightLevel)+_MinLightLevel;
...
void initBumpedNormalTangentBitangent(float4 normalMap1, float4 normalMap2, inout float3 bitangent, inout float3 tangent, inout float3 normal)
{
    float3 tangentNormal1 = UnpackScaleNormal(normalMap1, _BumpScale);
    float3 tangentNormal2 = UnpackScaleNormal(normalMap2, _BumpScale2);

    float3 tangentNormal = BlendNormals(tangentNormal1,tangentNormal2);
