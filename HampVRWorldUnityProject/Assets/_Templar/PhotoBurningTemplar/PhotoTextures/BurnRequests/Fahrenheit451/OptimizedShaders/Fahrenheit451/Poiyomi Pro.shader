Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/878a9a8434056cf4f943f8bebae8de50"
{
	Properties
	{
		[HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 8.1.111</color>", Float) = 0
 ...
 #define PROP_EMISSIONMAP 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0.282318,0,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0.282318,0,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(0)).rgb * poiThemeColor(poiMods, float4(1,0.282318,0,1).rgb, float(0));
				#endif
				if (float(0))
				{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(1,0,0.001597958,1).rgb, float(0)), float4(1,0,0.001597958,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(1,0,0.001597958,1).rgb, float(0)), float4(1,0,0.001597958,1).a);
				#endif
				...
				float rim = 1 - smoothstep(min(float(0.268), rimWidth), rimWidth, viewDotNormal);
				rim *= float4(1,0,0.001597958,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(1,0,0.001597958,1).rgb, float(0)));
				...
				float _RimLightPower_var = pow(_RimArea_var, exp2(lerp(3, 0, float(0.047))));
