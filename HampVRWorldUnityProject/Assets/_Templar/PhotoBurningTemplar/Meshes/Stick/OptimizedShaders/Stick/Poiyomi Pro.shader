Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Pro/a6ab24155e9edb842b678675ca1cdc90"
{
	Properties
	{
 ...
 #define _RIMSTYLE_POIYOMI 
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float rimWidth = lerp( - .05, 1, float(0.43));
				...
				#endif
				#if defined(PROP_RIMTEX) || !defined(OPTIMIZER_ENABLED)
				float4 rimColor = POI2D_SAMPLER_PAN(_RimTex, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)) * float4(poiThemeColor(poiMods, float4(1,0.2436173,0,1).rgb, float(0)), float4(1,0.2436173,0,1).a);
				#else
				float4 rimColor = float4(poiThemeColor(poiMods, float4(1,0.2436173,0,1).rgb, float(0)), float4(1,0.2436173,0,1).a);
				#endif
				...
				float rim = 1 - smoothstep(min(float(0.18), rimWidth), rimWidth, viewDotNormal);
				rim *= float4(1,0.2436173,0,1).a * rimColor.a * rimMask;
				if (float(0))
				{
					switch(float(0))
					{
			...
			void ApplyRimLighting(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#endif
				float3 rimColor = float3(poiThemeColor(poiMods, float4(1,0.2436173,0,1).rgb, float(0)));
